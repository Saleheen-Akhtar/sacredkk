...
add_action('wp_enqueue_scripts', function(): void {
    ...
    /* ── Inline CSS: override hardcoded upload URL for vedic/astrology background
       with the bundled theme asset. Must override all background sub-properties
       to prevent tiling from the original CSS declaration. ── */
    $vedic_bg_url = get_template_directory_uri() . '/assets/vedic-bg.png';
    $inline_css  = '.strip--circular {';
    $inline_css .= '  background-image: url(\'' . esc_url($vedic_bg_url) . '\') !important;';
    $inline_css .= '  background-size: cover !important;';
    $inline_css .= '  background-repeat: no-repeat !important;';
    $inline_css .= '  background-position: center center !important;';
    $inline_css .= '}';
    /* Also simplify the ::after overlay to a single clean 88% opacity ivory veil */
    $inline_css .= '.strip--circular::after {';
    $inline_css .= '  background: rgba(253,251,245,0.88) !important;';
    $inline_css .= '}';
    wp_add_inline_style('sacred-kompass-style', $inline_css);
